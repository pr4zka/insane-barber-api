...
let ComplaintsService = class ComplaintsService {
    ...
    async findById(id) {
        ...
        if (!reclamo) {
            throw new common_1.NotFoundException(`El reclamo #${id} no existe.`);
        }
        return reclamo;
    }
    async create(data) {
    ...
    async createFollowUp(reclamoId, data) {
        ...
        if (reclamo.estado === 'resuelto') {
            throw new common_1.BadRequestException('Este reclamo ya fue resuelto. No se pueden agregar mas seguimientos.');
        }
